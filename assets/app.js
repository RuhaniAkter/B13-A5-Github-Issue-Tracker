...
const getPriorityClasses = (priority) => {
  ...
  const classes = {
    ...
    MEDIUM:
      "bg-yellow-50 text-yellow-600 inset-ring inset-ring-yellow-500/10 dark:bg-yellow-400/10 dark:text-yellow-400 dark:inset-ring-yellow-400/20",
...
const showIssueDetails = async (issueId) => {
  ...
  let issue = appState.allIssues.find(
    (item) =>
  ...
  if (dialog.statusBadge) {
    dialog.statusBadge.innerHTML = isOpen ? `Open` : `Closed`;
...
const handleSearch = async () => {
  ...
  const searchResults =
    query === "" ? await fetchAllIssues() : await searchIssues(query);
